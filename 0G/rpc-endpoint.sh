...
RPC_PORT=$(grep -A 3 "\[rpc\]" $HOME/.0gchain/config/config.toml | grep -oP ":\K[0-9]+")
...
while [ $SUCCESS -eq 0 ]; do
    ...
    if [ "$SUCCESS" = "0" ]; then
        ...
        sudo systemctl restart 0gchaind &>/dev/null
        sleep 5
        ...
        if [ "$SUCCESS" = "0" ]; then
            PORT=$(grep -A 3 "\[rpc\]" $HOME/.0gchain/config/config.toml | egrep -o ":[0-9]+") && \
            PORT=${PORT#:} && \
