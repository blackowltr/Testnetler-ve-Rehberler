...
# Değişkenin Atanması
if [ ! $SEED ]; then
	read -p "İsminizi/Discord ID Yazın: " SEED
...
echo -e "\e[1m\e[32m2. Rust Yükleniyor... \e[0m" && sleep 1
# Rust Kurulumu
...
[Service]
User=$USER
ExecStart=/root/.cargo/bin/bazuka node --listen 0.0.0.0:8765 --external $IPADDR:8765 --network debug --db ~/.bazuka-debug $BOOTSTRAPT --discord-handle $SEED
Restart=on-failure
RestartSec=3
LimitNOFILE=65535
